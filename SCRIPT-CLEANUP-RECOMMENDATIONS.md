...
1. Remove misplaced markdown files from tofusoup/scripts (2 files)
1. Remove all __pycache__ directories from scripts/
1. **Estimated time:** 15 minutes
1. **Risk:** Very low (removing obviously stale files)

### Phase 2: Standardize mkdocs_hooks.py (Medium Risk)

1. Compare the 3 versions of mkdocs_hooks.py
1. If identical, move to central theme/hooks/
1. Test in terraform providers after centralization
1. **Estimated time:** 1 hour
1. **Risk:** Medium (need to verify builds still work)

### Phase 3: Review Documentation Scripts (Medium Risk)

1. Compare pyvider/check_doc_links.py with central docs_validate.py
1. Compare pyvider-components/validate_docs.py with central
1. Determine if unique logic exists or can consolidate
1. **Estimated time:** 1-2 hours
1. **Risk:** Medium (need to preserve unique functionality)

### Phase 4: Compare Before Extracting (Current Request)

1. Do NOT re-extract Makefiles yet
1. Compare current Makefiles with updated template
1. Identify differences that need to be preserved
1. Plan selective extraction
1. **Estimated time:** 30 minutes
1. **Risk:** Low (comparison only)
...
1. Review documentation script overlap
...
1. Selective extraction (not blanket re-extract)
